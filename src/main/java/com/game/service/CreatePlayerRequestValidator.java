...
public class CreatePlayerRequestValidator implements Validator {
    ...
    public boolean supports(Class<?> aClass) {
        return PlayerInfo.class.equals(aClass);
    }
    ...
    public void validate(Object o, Errors errors) {
        ...
        if (player.getProfession() == null) {
            errors.rejectValue("profession", "value must be not empty");
        }
        ...
        if (player.getBirthday() == null || player.getBirthday() < 0) {
        ...
        if (player.getRace() == null) {
            errors.rejectValue("race", "value must be not empty");
        }
    }
}
